...
class MemberLedgerController extends ChangeNotifier {
  MemberLedgerController({required MemberLedgerRepository repository})
  ...
  Future<void> load({MemberLedgerFilter? filter}) async {
    ...
    try {
      _statement = await _repository.statement(_filter);
    } on ApiException catch (error) {
      _errorMessage = error.message;
    } catch (_) {
      _errorMessage = 'Unable to load ledger statement. Please try again.';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
